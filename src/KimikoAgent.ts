...
class KimikoAgentConfig {
    ...
    public tool_choice?: { function?: { name: string }; type?: 'function' } | null
    public tools?: {
        function?: {
            description?: string
            name?: string
    ...
    public withFrequencyPenalty(frequency_penalty: number | null): KimikoAgentConfig {
        this.frequency_penalty = frequency_penalty;
        return this;
    }

    public withLogitBias(logit_bias: Record<string, number> | null): KimikoAgentConfig {
    ...
    public withParallelToolCalls(parallel_tool_calls: boolean | null): KimikoAgentConfig {
        this.parallel_tool_calls = parallel_tool_calls;
        return this;
    }

    public withPresencePenalty(presence_penalty: number | null): KimikoAgentConfig {
        this.presence_penalty = presence_penalty;
        return this;
    }

    public withResponseFormat(response_format: { type: 'json_object' } | null): KimikoAgentConfig {
    ...
    public withStreamOptions(stream_options: { include_usage?: boolean } | null): KimikoAgentConfig {
    ...
    public withToolChoice(tool_choice: { function?: { name: string }; type?: 'function' } | null): KimikoAgentConfig {
        this.tool_choice = tool_choice;
        return this;
    }

    public withTools(tools: { function?: { description?: string; name?: string; parameters?: Record<string, any> }; type: 'function' }[] | null): KimikoAgentConfig {
        this.tools = tools;
        return this;
    }
...
export class KimikoAgentBuilder {
    ...
    constructor(system_prompt: string, messages: LLM.ChoiceMessage[], model: string) {
    ...
    public build() {
        if (!this.system_prompt) {
            throw new Error('Required field missing: system_prompt must be set before building.');
        }
        if (!this.messages) {
            throw new Error('Required field missing: messages must be set before building.');
        }
        if (!this.model) {
            throw new Error('Required field missing: model must be set before building.');
        }
        ...
        return {
            send: async (): Promise<LLM.CompletionResponse> => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(`HTTP error! status: ${response.status}, payload: ${JSON.stringify(payload)}, endpoint: ${this.config.baseURL}`);
                    }
                    return await response.json();
                } catch (error: any) {
                    throw new Error(`Failed to send request to ${this.config.baseURL} with payload ${JSON.stringify(payload)}: ${error.message}`);
                }
            }
        };
    }
